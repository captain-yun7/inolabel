...
export default function SignaturesPage() {
  ...
  const handleSave = useCallback(async () => {
    ...
    if (duplicate) {
      alertHandler.showWarning(`${editingSignature.unit === 'excel' ? '엑셀부' : '스타부'}에 이미 ${editingSignature.sigNumber}번 시그가 있습니다.`, '중복 오류')
      return
    }
      ...
      if (targetItem) {
        ...
        if (duplicate) {
          alertHandler.showWarning(
            `${targetItem.unit === 'excel' ? '엑셀부' : '스타부'}에 이미 ${numValue}번 시그가 있습니다.`,
            '중복 오류'
          )
          return
        }
  ...
  const handleQuickAdd = useCallback(async () => {
    ...
    if (duplicate) {
      alertHandler.showWarning(
        `${activeUnit === 'excel' ? '엑셀부' : '스타부'}에 이미 ${quickAddData.sigNumber}번 시그가 있습니다.`,
        '중복 오류'
      )
      return
    }
                      ...
                      {videoUploadMode === 'url' ? (
                        <>
                          <input
                            ...
                            placeholder="SOOP VOD URL 또는 YouTube URL..."
                            style={{
                              width: '100%',
                              padding: '8px 10px',
                              border: '1px solid var(--card-border)',
