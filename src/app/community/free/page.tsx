...
export default function FreeBoardPage() {
  ...
  // 공지 & BEST 게시글
  const [pinnedNotices, setPinnedNotices] = useState<{ id: number; title: string; createdAt: string }[]>([])
  ...
  useEffect(() => {
    async function loadExtras() {
      const [noticeResult, weeklyResult, monthlyResult] = await Promise.all([
        getNotices({ limit: 3 }),
                  ...
                  <div className={styles.bestColumn}>
                    ...
                    <ul className={styles.bestList}>
                      {weeklyBest.map((post, i) => (
                        <li key={post.id}>
                          <Link href={`/community/free/${post.id}`} className={styles.bestItem}>
