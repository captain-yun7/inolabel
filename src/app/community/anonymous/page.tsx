...
export default function AnonymousBoardPage() {
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === sortedPosts.length) setSelectedIds(new Set())
  ...
  const handleDeleteSelected = async () => {
    ...
    if (result.error) {
      ...
      alert(failed > 0 ? `${deleted}개 삭제 완료, ${failed}개 삭제 실패` : `${deleted}개 게시글이 삭제되었습니다.`)
      setSelectedIds(new Set())
      ...
      <div className={styles.main}>
        ...
        <section className={styles.hero}>
          <div className={styles.heroContent}>
            <h1 className={styles.title}>익명게시판</h1>
            <p className={styles.subtitle}>익명으로 자유롭게 소통하는 공간</p>
          </div>
        </section>
          ...
          <div className={styles.boardHeader}>
            ...
            <div className={styles.searchArea}>
              ...
              <div className={styles.searchBox}>
                ...
                <button className={styles.searchBtn} onClick={() => { setDebouncedSearch(searchQuery); setCurrentPage(1) }}>
                  <Search size={16} />
                </button>
              </div>
            </div>
          ...
          {error ? (
            ...
            <>
              ...
              <div className={styles.boardFooter}>
                <div />
                <Link href="/community/write?board=anonymous" className={styles.writeBtn}>
                  <PenLine size={16} /> 글쓰기
                </Link>
              </div>
            </>
            ...
            <>
              <div className={`${styles.board} ${isSelectMode ? styles.selectMode : ''}`}>
                  ...
                  {isSelectMode && (
                    <span className={styles.colCheck}>
                      <button onClick={toggleSelectAll} className={styles.checkBtn}>
                        {selectedIds.size === sortedPosts.length ? <CheckSquare size={16} className={styles.checkedIcon} /> : <Square size={16} />}
                      </button>
                    </span>
                  )}
                ...
                <div className={styles.tableBody}>
                  {sortedPosts.map((post, index) => (
                    <Link key={post.id} href={`/community/anonymous/${post.id}`} className={`${styles.row} ${selectedIds.has(post.id) ? styles.selected : ''}`}>
                      {isSelectMode && (
                        <div className={styles.cellCheck} onClick={(e) => toggleSelect(post.id, e)}>
                          {selectedIds.has(post.id) ? <CheckSquare size={16} className={styles.checkedIcon} /> : <Square size={16} />}
                        </div>
                      )}
                      <div className={styles.cellNumber}>{sortedPosts.length - index}</div>
                      <div className={styles.cellCategory}><span className={styles.categoryBadge}>익명</span></div>
                      ...
                      <span className={styles.cellAuthor}>
                        익명
                        {isAdmin && post.authorRealName && <span className={styles.adminRealName}>({post.authorRealName})</span>}
                      </span>
              ...
              <div className={styles.mobileList}>
                {sortedPosts.map((post) => (
                  <Link key={post.id} href={`/community/anonymous/${post.id}`} className={styles.mobileCard}>
                    <div className={styles.mobileHeader}>
                      <span className={styles.mobileCategoryBadge}>익명</span>
                      {isNew(post.createdAt) && <span className={styles.newBadge}>N</span>}
                      {isHot(post.viewCount, post.commentCount, post.likeCount) && <span className={styles.hotBadge}>HOT</span>}
                    </div>
                    <h3 className={styles.mobileTitle}>
                      {post.title}
                      {post.commentCount > 0 && <span className={styles.mobileCommentCount}>[{post.commentCount}]</span>}
                    </h3>
                    <div className={styles.mobileMeta}>
                      <span className={styles.mobileAuthor}>익명</span>
                      <span className={styles.mobileDivider}>·</span>
                      <span>{formatShortDate(post.createdAt)}</span>
                      <span className={styles.mobileDivider}>·</span>
                      <span className={styles.mobileStats}><Eye size={12} /> {post.viewCount}</span>
              ...
              <div className={styles.boardFooter}>
                <div className={styles.pagination}>
                  ...
                  {getPageNumbers().map(pageNum => (
                    <button key={pageNum} className={`${styles.pageBtn} ${currentPage === pageNum ? styles.active : ''}`} onClick={() => setCurrentPage(pageNum)}>{pageNum}</button>
                  ))}
